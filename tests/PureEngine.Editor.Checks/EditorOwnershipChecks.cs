...
static class EditorOwnershipChecks
{
    ...
    public static void Run(string root)
    {
        ...
        var originalYaml = new SceneSerializer(ComponentAssets.Registry).Serialize(scene);
        ...
        var restored = new SceneSerializer(ComponentAssets.Registry).Deserialize(File.ReadAllText(path));
        ...
        Check(ComponentAssets.TryAttach(target, typeof(OwnershipProbe), services.Factory), "Removed types must be attachable again.");
        ...
        Check(target.Components.Count == 0 && cards.Children.Count == 0
            && !editor.FindControl<StackPanel>("AttachedClasses")!.IsVisible, "The last component must be removable.");
