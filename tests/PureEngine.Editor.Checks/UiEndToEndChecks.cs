...
internal static class UiEndToEndChecks
{
    public static void Run(string root)
    {
        ...
        using (var editServices = PureEngine.Runtime.GameSession.Create(GameServices.ForProject(components)))
        {
            ...
            Check(components.TryAttach(card, typeof(PureEngine.Core.Image), editServices.Factory), "E2E Image add failed.");
        }
        ...
        child.Attach(new PureEngine.Core.Image { Sprite = new Sprite(imported.Id), Order = -1 });
        ...
        using (var session = ProjectSession.Open(project.ManifestPath))
        {
            ...
            Check(!ReferenceEquals(cloneCard, reopened)
                && !ReferenceEquals(cloneCard.GetComponent<PureEngine.Core.Image>(), reopened.GetComponent<PureEngine.Core.Image>())
            ...
            Check(cloneCard.GetComponent<PureEngine.Core.Image>()!.Order == 3
                && clone.Objects.First(item => item.Name == "Badge").GetComponent<PureEngine.Core.Image>()!.Order == -1,
