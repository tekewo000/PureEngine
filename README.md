...
- 辞書キーの改名は入力後にEnter、または欄からフォーカスを移して確定する。Escで元のキーに戻る。空・重複キーや未確定の改名がある間は保存を止め、既存の値とMissing情報は確定まで変更しない。
...
- Inspectorの参照欄は1行で、名前だけの選択肢と右端の×（Clear）を表示する。ID・フルパスはツールチップに移し、2行目はMissing・旧インライン値の注意だけに使う。SceneObject／登録Componentの欄にはStuffsの行または型が一致するPrefabファイル、DataAsset型の欄には型が一致するDataAssetファイルをドラッグ＆ドロップできる（Componentアタッチとは別形式）。Prefabを落とした場合は生成元のRootまたは一致するComponentへの参照だけを割り当て、Hierarchyには生成しない。×で解除できる。対象がない、または候補が複数の場合は選ばない。Play中は編集できない。変更したときだけ未保存になる。
- 参照を設定したいオブジェクトを選択し、参照先のStuffs行をInspectorの参照欄へドラッグする。行を押した時点では選択を切り替えず、ドラッグ中もInspectorを維持する。ドラッグせずに離すと、その行を選択する。
- 対象を削除・取り外すとC#はnullになり、IDはMissingとして保持される。Missingのまま保存・開き直しができ、同じIDが戻れば再接続する。別の対象を選び直すと実物の値を優先する。Missingを消すときは欄の×（Clear）を使う。単なるnull代入ではMissingは消えない。
...
- 参照を含む配列・List・stringキーDictionaryは、埋め込みクラスの中でも選択・解除・行の追加／削除ができる。見出しは枠なしアイコンのAdd・Set Null・一括Clear。Inspectorで行削除や辞書キー変更を行うとMissingのIDも移動する。Missingを含むコレクションを通常のC#から構造変更する場合は `Scene.References` の明示操作で保持パスも更新する（null同士の移動は通常のC#参照から判別できない）。
...
- Recent Projectsはダブルクリック、または選択してEnterで開く。履歴は最大12件を `%LOCALAPPDATA%/PureEngine/recent-projects.json` に保存し、Projectフォルダには含めない。各行のRemoveボタン、または選択してDelete／Backspaceで履歴からのみ削除できる（Projectフォルダ自体は消えない）。
...
- 値の規則はシーンと同じで、`[Inspector]` の付いた対応型だけを保存します。`SceneObject` やデータアセット型への参照は保存できません。登録済みの普通のクラス（`Transform` を含む）は入れ子の値として保存します。
- 入れ子のデータは自作struct（Nullableを含む）や既存互換の普通のクラスで書けます。シーンでは参照として扱われる登録済みクラスも、アセット内ではファイルが所有する値として保存します。新しいclass埋め込み指定はなく、設定データの共有には既存のDataAssetを使います。
...
- 同じ型のアセットをまとめて編集するには、Game横の **Data Assets** タブを使います。上部で型を選ぶと同型の全ファイルが行に並び、スカラーは直接、`List`・`Dictionary`・入れ子クラスは箱内のスクロールで編集します。行は既定固定高で、下辺ドラッグで一時的に伸ばせます。**Save All**（表選択中のCtrl+S）で一括保存し、入力エラーがある間は全体を保存しません。行の追加・複製・削除・単体Inspectorでの表示にも対応し、Play中は表ごと読み専になります。
...
SceneObjectを1つ選び、子孫ごとファイル化して何度でも複製できます。コピーのみで、元の編集は配置済みへ自動反映されません。リンク・個別Override・Variantはありません。保存元・配置したルートとPrefab EditorのルートはStuffsでPrefabアイコンになり、子や通常オブジェクトはSceneObjectアイコンのままです。

- Stuffsで対象を選んで右クリック→ **Save as Prefab…** で保存します。またはStuffsの行をProject欄へD&Dしても同じ保存になります。保存先はメニューでは表示中のフォルダ、D&Dでは落としたフォルダで、ファイルは `.pure.prefab.yaml` です。いずれも既存ファイルの上書きはせず、D&Dでは自動連番で作成します。保存元のルートはPrefabアイコンに変わり、シーンは未保存になります。
- Project欄のPrefabファイルをダブルクリック／Enter、または右クリック→ **Open** で中央の **Prefab Editor** タブに開きます。開けるPrefabは同時に1つです。この操作ではシーンに配置しません。既存のペイン位置は変えず、StuffsとInspectorは選択中のScene／Prefab Editorタブの内容を表示します。
- Prefabはメインシーンと分離したデータで編集します。タブを切り替えても両方の編集状態を保持し、Prefab Editorで **Ctrl+S** を押すと元ファイルへ手動保存します（自動保存はしません）。Prefab・既存Object・ComponentのIDは保存しても維持します。タブを閉じる、別のPrefabで置き換える、開いているファイルや親フォルダを改名・削除するときは、未保存の変更を保存／破棄／キャンセルから選びます。
- Prefabのルートは1つに保ちます。ルートの削除・親変更や、子をルート外へ移す操作はできません。未選択での追加はルートの子になります。別ファイルを作る場合はStuffsの **Save as Prefab…** またはProjectへのD&Dを使います（Prefab EditorでのCtrl+Shift+Sは元ファイルを上書きしません）。
- 配置はProject欄のファイルを右クリック→ **Place in Scene**、またはStuffsへのD&Dです。Sceneタブで操作し、行の上ならその子、余白ならルートの末尾に置きます（メニューはStuffsの選択を親にします）。Prefab内の兄弟順を保ちます。配置したルートはPrefabアイコンになり、シーンが未保存になります。Prefabを編集・保存しても配置済みコピーは自動更新されず、リンクやOverrideも作りません（表示用の配置元IDのみ記憶します）。
- PlayはPrefabではなくメインシーンを実行します。Prefabを開いている間はC#のホットリロードを保留し、ステータスにタブを閉じて反映する案内を表示します。コンパイル失敗の診断はPrefabを閉じる前でも表示します。
...
Project ExplorerでC#ファイルをダブルクリック／Enter、または右クリック→ **Open** すると、ProjectのルートとそのファイルをZedで開きます（ルートをワークスペースとして開き、ファイルを表示します）。Play中も開けます。`zed` コマンドがPATHにない場合はステータスとConsoleに理由を表示します。WindowsではZedのインストールフォルダをPATHに追加してください。
...
ツールバーのPlayは編集中Sceneの複製で `PlaySession` を作り、約60Hzのタイマーで実測の経過秒を渡して更新します。Play開始でGameタブへ切り替え、Stopや自動停止でScene Viewタブへ戻ります。Inspectorに入力エラーがある間は開始せず、画面下部に理由を表示します。実行中はシーン編集・切替とシーン操作メニューを無効化し、Stopで終了します。開始・更新・終了の失敗とSceneRuntimeのErrorsは画面下部に表示し、失敗後も操作可能な状態へ戻します。ウィンドウを閉じる際も実行中なら終了・解放します。再Playは新しいSceneRuntimeで開始し、Gameタブに実行用Sceneを描画します。
