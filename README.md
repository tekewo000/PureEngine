...
- 参照を含む配列・List・stringキーDictionaryは、埋め込みクラスの中でも選択・解除・行の追加／削除ができる。見出しはAdd・Set Nullと目立たないゴミ箱の一括Clear。Inspectorで行削除や辞書キー変更を行うとMissingのIDも移動する。Missingを含むコレクションを通常のC#から構造変更する場合は `Scene.References` の明示操作で保持パスも更新する（null同士の移動は通常のC#参照から判別できない）。
