...
- Stuffsで対象を選んで右クリック→ **Save as Prefab…** で保存します。またはStuffsの行をProject欄へD&Dしても同じ保存になります。保存先はメニューでは表示中のフォルダ、D&Dでは落としたフォルダで、ファイルは `.pure.prefab.yaml` です。いずれも既存ファイルの上書きはせず、D&Dでは自動連番で作成します。
...
- 配置はProject欄のファイルを右クリック→ **Place in Scene**、またはStuffsへのD&Dです。Sceneタブで操作し、行の上ならその子、余白ならルートの末尾に置きます（メニューはStuffsの選択を親にします）。Prefab内の兄弟順を保ちます。配置後はシーンが未保存になります。Prefabを編集・保存しても配置済みコピーは自動更新されず、リンクやOverrideも作りません。
