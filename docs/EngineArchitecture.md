...
- Parent・Priority・参照・Editor設定はそれぞれの機能を実装するときに追加する。
...
- Parent、ライフサイクルの実行とPriority。
- ゲーム実行、描画、ゲームのセーブデータ、通信・Steam連携。

アタッチ・取得、属性の検出、Editorでのドラッグ＆ドロップ・Inspectorの値編集、YAMLシーン保存・復元まで実装済み。
今後はライフサイクルの実行とPriorityへつなげる。
