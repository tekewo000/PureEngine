...
- Editor配置とゲーム実行中の動的生成は同じ配置経路を使う。EditorはStuffsの右クリック保存・Projectペインの管理（作成相当の一覧・改名・削除）・明示的な配置メニュー・Project→StuffsへのD&D配置（ファイルパス単位のペイロード、行上はその子・余白はルート、行のハイライト付き）・Play中の保存・配置・D&D禁止に対応する。新規保存先はProject内の `.pure.prefab.yaml` で、Save as Prefabは既存ファイルの上書きをしない。InspectorのSceneObject／登録Component参照欄へのD&Dは後述の非実行テンプレート参照であり、配置はしない。
