...
- Game横のLocalizationタブで行の追加・改名・削除と言語列の追加・削除、各セルの本文・ボイス編集を行う。重複・空キーがある間は保存と終了を止める。`Text.LocalizedEntry` 参照で解決文を描き、直書き `Content` は代替に残る。Inspectorはキー一覧からの選択・Clear・Missing表示だけで、本文・改名の編集は置かない。
- 解決の正本はCoreの `LocalizationService`（現在言語→既定→最初→呼び出し側代替、小文字正規化・不正値拒否）。編集・各Playは独立した `LocalizationStore` スナップショットを持ち、ゲームコードはサービスとストアのコンストラクタ注入で受ける。Play開始時はプレビュー言語を引き継ぎ、実行中の切替はその実行だけに効く。未保存の表がある間はPlayを開始しない。プレビュー言語はツールバー選択でScene View／Gameに反映し、保存しない。
- 保存するのは行の不変ID（`{loc: id}`）のみで、表示キー名の改名では壊れない。欠落はID保持＋診断、Clone・Playはスナップショット分離。ボイス欄は音声基盤までの予約で文字列のまま保存・編集し、`ResolveVoice`（空は無音）までを提供する。
- `LocalizationChecks`（表の保存往復・正規化診断・解決行列・言語検証・シーン保存/Clone・Play分離・欠落/復旧・不正形式拒否）と `LocalizationEditorChecks`（ツールバー選択・タブ編集/保存/再読込・未保存Play抑止・キー選択・プレビュー/Game解決・空描画）を追加。Prefabタブの固定indexを使う既存検査をタブ挿入に合わせて更新した。
...
- 起動時のコンパイル状態を保存監視へ引き継ぎ、未変更の構文木・参照DLL・前回コンパイルを再利用する。入力が同じ保存ではコンパイル・読み込み・移行を省略し、Play中などの反映待ちを維持する。
- 実行は最大1件、待機は最新1件に集約し、古い実行へキャンセルを通知する。保存後の待機は600msから250msへ短縮する。DLLは全体生成・差し替えを維持し、ファイル分割やHot Reloadは行わない。
- 回帰チェック（無変更スキップ、1ファイル変更の再利用、同長変更の検出、追加・削除・移動、全削除、依存破壊の検出、失敗後の復帰、直列化と最新集約、待機時間）を `UserCodeIncrementalChecks` に追加し、重なり・切替・終了の既存検査を直列化に合わせて更新した。
- ローカルの `./tools/code-quality.ps1 -Check`（提案レベル解析・警告をエラー扱いにしたビルド・Core/Editor Checks）は通過。実画面の保存体感・CIは未確認として区別する。設計は[ProjectのC#再読み込み](EngineArchitecture.md#projectのc再読み込み)を参照。
...
- 実装範囲：Projectのダブルクリック／Enter／Openから中央の単一Prefab Editorタブを開く。既存ペインの位置を変えず、Stuffs／Inspectorはアクティブタブの編集コンテキストを共有する。メインシーンとPrefabは分離した編集データを持ち、タブ切替でそれぞれの状態を保持する。
- Ctrl+Sで元Prefabへ手動保存し、Prefab・既存Object・ComponentのIDを維持する。閉じる／置換時の未保存確認は保存・破棄・キャンセル。Explorerの改名・削除は開いているPrefabまたは祖先フォルダが対象ならディスク変更前に閉じる確認を行い、メインシーンのパス・Startup管理はPrefabと分離する。
- 終了確認は全編集文書の確認が済むまで破棄を確定しない。Prefab置換は候補の読み込み・検証を先に行い、失敗時は編集中のPrefab／データアセットを保持する。Inspectorの入力エラーを編集対象で区別し、Prefab編集中のSave Asショートカットでは元ファイルを上書きしない。
- ダブルクリックでは配置せず、明示メニューとStuffsへのD&Dを残す。Playはメインシーンを実行し、Prefabが開いている間のコード反映は保留してステータスで案内する。配置済みコピーの自動更新・Overrideは対象外。操作は[README](../README.md#プレハブを使う)、設計は[Prefabs節](EngineArchitecture.md#prefabs)を参照。
- 検証：ローカルの `./tools/code-quality.ps1 -Check`（提案レベル解析・警告をエラー扱いにしたビルド・Core/Editor Checks）が通過。Headlessでペインの親／Grid位置不変、ダブルクリック、Inspector編集、保存先とdirtyの分離、ID維持、選択／視点復元、ルート保護、Play、保存／破棄／キャンセル、複数文書の終了キャンセル、無効ファイルでのデータアセット保持、Play後の保存先、Save As誤上書き防止、Inspector例外時の候補破棄を確認した。実画面はGUI操作ツールのランタイム接続が利用できず未確認。CIも未確認。
...
- 一覧編集と入れ子値の所有（2026-09-25実装）：Game横のData Assetsタブで同型の全アセットを行編集する。セルは単体Inspectorの行エディター流用で、コレクション・入れ子単体は箱＋内スクロール、行高は既定固定＋下辺ドラッグの一時伸縮とする。表全体の一括dirty・一括保存・入力エラーとPlay中の全体停止・型切替と終了時の保存確認・C#再反映の付け替えに対応する。Coreはアセット所有の判定を `SceneReferenceTypes.ContainsAssetExternalReference` に一本化し、`SceneObject` とデータアセット型の参照だけを拒否して登録済み通常クラス・`Transform` を入れ子値として保存する（YAML形式と版は変更なし）。回帰チェック（Coreの登録済み入れ子の受容と往復・SceneObject参照の拒否、Editorの一括編集と保存・無効ガード・入れ子とコレクションセル・行の追加と削除・再反映の列組み直し・Playロック）を追加し、ローカルの `./tools/code-quality.ps1 -Check` は通過。実画面・CIは未確認として区別する。
...
| Project Explorer | フォルダツリーとファイル一覧、シーンを開く、作成・改名・削除・更新、D&D移動、名前指定で空のsealedクラスを作るCreate C#。組み込みComponents一覧は表示しない。エディタが扱う種類（シーン／C#／画像／データアセット／Prefab／Localization）のみ表示し、管理情報・生成物・一般ファイルは隠す | [MainWindow.ProjectExplorer](../src/PureEngine.Editor/Windows/MainWindow.ProjectExplorer.cs)、[MainWindow.ProjectDrop](../src/PureEngine.Editor/Windows/MainWindow.ProjectDrop.cs) |
...
| Localization | プロジェクト直下の1表（`Localization.pure.loc.yaml`）。テーブル全体で統一した言語列、行の表示キー名・言語ごとの本文・ボイス欄。Localizationタブのグリッド編集、`Text.LocalizedEntry` 参照の解決描画、ツールバーの言語選択、編集・Play別の `LocalizationStore` スナップショット、ID保存・Missing保持・Clone分離、未保存Play抑止 | [LocalizationTable](../src/PureEngine.Core/Localization/LocalizationTable.cs)、[LocalizationService](../src/PureEngine.Core/Localization/LocalizationService.cs)、[Text](../src/PureEngine.Core/Components/Text.cs)、[UiTextRenderer](../src/PureEngine.Rendering/UiTextRenderer.cs)、[LocalizationViewModel](../src/PureEngine.Editor/ViewModels/LocalizationViewModel.cs) |
...
### Localization（2026-09-26、単一テーブルへ移行）

- `LocalizationChecks` で表の保存往復（キー・多言語本文・ボイス欄・Unicode）・正規化診断（不正言語・重複行・表外セル言語の破棄）・解決行列（現在→既定→表順最初→代替、空欄スキップ）・言語検証（BCP47式・不正値拒否・正規化）・シーン保存（IDのみ）／復元（ID保持）／Clone分離・Play分離・欠落（ID保持）／復旧・不正形式拒否を確認。
- `LocalizationEditorChecks` でツールバー選択の初期値・無効状態・タブの行/言語追加とセル編集・保存と再読込・未保存Play抑止・保存後Playと言語引継ぎ・キー選択候補・シーン保存のID記録・プレビュー/Game解決と代替・空描画を確認。タブ挿入に伴うPrefabタブindexの既存検査を更新した。新規文言の手動作成フロー・言語列グリッドの実画面・ボイス再生・実GPU・CIは未確認として区別する。
