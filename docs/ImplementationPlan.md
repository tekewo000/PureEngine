...
- `InspectorValueTypes.cs` は内容・namespaceを変えず `Components/` から `Values/` へ移動。今回のColorは単独の値型で、既存の `Image.Color`（Vector4）への接続は含めない。Inspector・YAML・Cloneには `Color` を新規対応する。
