...
実装済みと動作確認済みは区別する。以下の次工程は提案であり、着手・仕様確定を意味しない。
...
- [ ] 検出済みメソッドをどう呼ぶか、不正な宣言や実行時例外をどう報告するかを決める。
- [ ] 開始・停止・再開時のStart／Destroy、実行中の追加・削除の扱いを決める。
- [ ] 編集用Sceneと実行用Sceneを分け、実行中の変更を制作データへ自動反映しない方針を具体化する。
