...
public partial class MainWindow
{
    ...
    {
        if (_project is null || !e.DataTransfer.Formats.Contains(DataFormat.File))
        {
    ...
    private async Task ImportDroppedStorageItemsAsync(string targetRelative, IStorageItem[] items)
    {
        ...
        if (localPaths.Count > 0)
            imported.AddRange(ProjectFileImporter.ImportLocalPaths(targetDir, localPaths));
    ...
    private static async Task<string> ImportRemoteFileAsync(IStorageFile source, string targetDirectory)
    {
        ...
        var destination = ProjectFileImporter.GetUniqueDestinationPath(targetDirectory, source.Name);
        await using var read = await source.OpenReadAsync();
        await using var write = File.Create(destination);
